...
async function import_browserfs() {
    ...
    script.src = new URL("browserfs.min.js", import.meta.url).href
    console.warn("late import", script.src)
...
const LOCAL_PKG_REPO = "/play/cdn/repo/"
const PYGAME_PKG_CDN = "https://pygame-web.github.io/archives/repo/"

function rewritePkgUrl(url) {
    if (typeof url === "string" && url.indexOf(PYGAME_PKG_CDN) === 0) {
        return LOCAL_PKG_REPO + url.slice(PYGAME_PKG_CDN.length)
    }
    return url
}
...
window.fetch = function(input, init) {
    if (typeof input === "string") {
        input = rewritePkgUrl(input)
    } else if (input && input.url) {
        const rewritten = rewritePkgUrl(input.url)
...
async function custom_postrun() {
    ...
    const pyrc_url = new URL("pythonrc.py", import.meta.url).href
    var content = 0
...
window.cross_track = async function cross_track(trackid, url, flags) {
    url = rewritePkgUrl(url)
    var response = await fetch(url, flags || FETCH_FLAGS);

    if (!checkStatus(response) || !response.body) {
        const trackErr = MM[trackid]
        trackErr.pos = -1
        trackErr.error = true
        console.error("1396: cannot download", url, response && response.status)
        return
    }
    ...
    while(true) {
        ...
        track.pos = received
        if (received > track.len) {
            track.len = received
        }
    }

    track.data = new Uint8Array(received)
    let offset = 0
